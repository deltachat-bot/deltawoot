...
def test_send_message(delta, woot, lp):
    ...
    dchat = delta.secure_join(woot.invite)
    delta.wait_for_securejoin_joiner_success()
    help_msg = delta.wait_for_incoming_msg().get_snapshot()
    assert help_msg.text == DEFAULT_HELP_MSG % help_msg.sender.get_snapshot().auth_name
    ...
    while len(woot.get_messages(wconversation)) < 1:
...
def test_leave_groups(delta, woot, lp):
    ...
    delta.secure_join(woot.invite)
    delta.wait_for_securejoin_joiner_success()
    help_msg = delta.wait_for_incoming_msg().get_snapshot()
    assert help_msg.text == DEFAULT_HELP_MSG % help_msg.sender.get_snapshot().auth_name
    bot_contact = help_msg.sender
    dgroup.add_contact(bot_contact)
    ...
    delta.wait_for_event(EventType.CHAT_MODIFIED)
